...
$eid=intval($_GET['eid']);
$status="0";
$sql = "UPDATE pato_testimonial SET status=:status WHERE  id=:eid";
...
$sql = "UPDATE pato_testimonial SET status=:status WHERE  id=:aeid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
...
<head>
	...
	<title>Tutsy Delights | Manage testimonials   </title>
						...
						<div class="panel panel-default">
							...
							<?php if($error){?><div class="errorWrap"><strong>ERROR</strong>:<?php echo htmlentities($error); ?> </div><?php } 
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
									...
									<?php $sql = "SELECT users.name,pato_testimonial.UserEmail,pato_testimonial.Testimonial,pato_testimonial.PostingDate,pato_testimonial.status,pato_testimonial.id from pato_testimonial join pato_users on pato_users.Emailid=pato_testimonial.UserEmail";
$query = $dbh -> prepare($sql);
$query->execute();
